...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {broken.length > 0 && (
        <div className="flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">
          <div className="flex items-center gap-3">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/50">
              <Activity className="h-4 w-4" />
            </span>
            <div>
              <p className="text-sm font-bold">{broken.length} apps currently broken</p>
              <p className="text-xs opacity-80">Maintenance required to restore these services.</p>
            </div>
          </div>
          <Button variant="outline" size="sm" asChild className="border-red-200 hover:bg-red-100 dark:border-red-900">
            <Link href="/apps?status=broken">View list</Link>
          </Button>
        </div>
      )}
